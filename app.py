...
def main():
    ...
    with right:
        ...
        parch_slider = st.slider(
            "# Liczba rodziców i/lub dzieci", min_value=0, max_value=6
        )
        fare_slider = st.slider("Cena biletu", min_value=0, max_value=500, step=10)

    male = 1 if sex_radio == 1 else 0

    q = 1 if embarked_radio == "Q" else 0
    s = 1 if embarked_radio == "S" else 0
    ...
    with prediction:
        st.header(
            "Czy dana osoba przeżyje? {0}".format("Tak" if survival[0] == 1 else "Nie")
        )
        st.subheader(
